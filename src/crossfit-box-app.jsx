...
export default function CrossFitBoxApp() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      const existingUser = await window.storage.get(`user:${signupEmail.toLowerCase()}`, true);
      if (existingUser) {
      ...
      // Save user
      await window.storage.set(`user:${signupEmail.toLowerCase()}`, JSON.stringify(user), true);
  ...
  const handleLogin = async () => {
    ...
    try {
      const result = await window.storage.get(`user:${loginEmail.toLowerCase()}`, true);
      if (result) {
  ...
  const postWOD = async () => {
    ...
    try {
      await window.storage.set(`wod:${wodToPost.date}:${wodToPost.group}`, JSON.stringify(wodToPost), true);
  ...
  const loadTodayWOD = async () => {
    ...
    try {
      // Try combined first
      let result = await window.storage.get(`wod:${today}:combined`, true);
      if (!result && currentUser.group) {
        // Try user's specific group
        result = await window.storage.get(`wod:${today}:${currentUser.group}`, true);
      }
  ...
  const logResult = async () => {
    ...
    try {
      await window.storage.set(`result:${resultId}`, JSON.stringify(result), true);
  ...
  const loadMyResults = async () => {
    try {
      const result = await window.storage.list('result:', true);
  ...
  const loadAllWODs = async () => {
    try {
      const result = await window.storage.list('wod:', true);
      if (result && result.keys) {
  ...
  const confirmDeleteWOD = async () => {
    ...
    try {
      ...
      if (coachResult) {
        await window.storage.delete(`result:${coachResult.id}`, true);
      }
      
      // Delete the WOD
      await window.storage.delete(`wod:${wod.date}:${wod.group}`, true);
  ...
  const deleteWorkout = async (workoutId) => {
    try {
      await window.storage.delete(`result:${workoutId}`, true);
      await loadMyResults();
      if (currentUser.role === 'coach') {
        await loadAllResults();
      }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-slate-800 rounded-lg p-8 border border-slate-700">
          <div className="text-center mb-8">
            <BBoxLogo className="w-24 h-12 mx-auto mb-3" />
            <h1 className="text-3xl font-bold text-white mb-2">BBOX CrossFit</h1>
            <p className="text-slate-400">Workout Logger</p>
          </div>
          ...
          {!showSignup ? (
            <>
              ...
              <button
                ...
                className="w-full bg-red-600 hover:bg-red-700 disabled:bg-red-800 disabled:cursor-not-allowed text-white font-semibold py-3 rounded-lg mt-4 mb-3 transition-colors flex items-center justify-center gap-2"
              >
                {authLoading ? (
                  <>
            ...
            <>
              ...
              <div className="space-y-3">
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Full Name <span className="text-red-500">*</span></label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Email <span className="text-red-500">*</span></label>
                  <input
                    type="email"
                    placeholder="you@example.com"
                    value={signupEmail}
                ...
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Password <span className="text-red-500">*</span></label>
                  <input
                    type="password"
                ...
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Confirm Password <span className="text-red-500">*</span></label>
                  <input
                    type="password"
                ...
                <div>
                  <label className="block text-slate-400 text-sm mb-1">Training Group</label>
                  <select
                    value={signupGroup}
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
        <div className="max-w-4xl mx-auto pb-20">
          {/* Header */}
          <div className="sticky top-0 bg-slate-900/95 backdrop-blur-sm z-10 p-4 border-b border-slate-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <BBoxLogo className="w-12 h-6" />
                <div>
                  <h1 className="text-2xl font-bold text-white">BBOX</h1>
            ...
            {coachView === 'dashboard' && (
              <>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-xl p-5 border border-red-500 shadow-lg">
                    <div className="text-red-100 text-xs font-medium mb-1">TOTAL</div>
                    <div className="text-3xl font-bold text-white mb-1">{stats.totalWorkouts}</div>
                    <div className="text-red-200 text-xs">Workouts</div>
                  </div>
                  <div className="bg-gradient-to-br from-orange-600 to-orange-700 rounded-xl p-5 border border-orange-500 shadow-lg">
                    <div className="text-orange-100 text-xs font-medium mb-1">THIS WEEK</div>
                  ...
                  <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-5 border border-blue-500 shadow-lg">
                    <div className="text-blue-100 text-xs font-medium mb-1">THIS MONTH</div>
                    <div className="text-3xl font-bold text-white mb-1">{stats.thisMonth}</div>
                    <div className="text-blue-200 text-xs">Workouts</div>
                  </div>
                  <div className="bg-gradient-to-br from-purple-600 to-purple-700 rounded-xl p-5 border border-purple-500 shadow-lg">
                    <div className="text-purple-100 text-xs font-medium mb-1">THIS YEAR</div>
                    <div className="text-3xl font-bold text-white mb-1">{stats.thisYear}</div>
                    <div className="text-purple-200 text-xs">Workouts</div>
                  </div>
                </div>
                  ...
                  <div className="bg-slate-800 rounded-xl p-5 mb-6 border border-slate-700 shadow-lg">
                        ...
                        {todayWOD.movements.map((movement, idx) => (
                          <div key={idx} className="flex items-start gap-3">
                            ...
                            <div className="flex-1">
                              ...
                              {movement.notes && (
                                <div className="text-slate-400 text-xs mt-0.5">{movement.notes}</div>
                              )}
                            </div>
                          </div>
                        ))}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        ...
                        className="bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-lg font-semibold transition-colors text-sm flex items-center justify-center gap-2"
                      >
                        {myResult.existingResultId ? (
                          <>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-slate-900 border-t border-slate-700 safe-area-inset-bottom">
            <div className="max-w-4xl mx-auto grid grid-cols-5 text-center">
              <button
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <BBoxLogo className="w-12 h-6" />
              <h1 className="text-4xl font-bold text-white">My Workouts</h1>
            </div>
        ...
        {currentView === 'dashboard' && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              <div className="bg-gradient-to-br from-red-600 to-red-700 rounded-xl p-5 border border-red-500 shadow-lg">
                <div className="text-red-100 text-xs font-medium mb-1">TOTAL</div>
                <div className="text-3xl font-bold text-white mb-1">{calculateStats().totalWorkouts}</div>
                <div className="text-red-200 text-xs">Workouts</div>
              </div>
              <div className="bg-gradient-to-br from-orange-600 to-orange-700 rounded-xl p-5 border border-orange-500 shadow-lg">
                <div className="text-orange-100 text-xs font-medium mb-1">THIS WEEK</div>
              ...
              <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl p-5 border border-blue-500 shadow-lg">
                <div className="text-blue-100 text-xs font-medium mb-1">THIS MONTH</div>
                <div className="text-3xl font-bold text-white mb-1">{calculateStats().thisMonth}</div>
                <div className="text-blue-200 text-xs">Workouts</div>
              </div>
              <div className="bg-gradient-to-br from-purple-600 to-purple-700 rounded-xl p-5 border border-purple-500 shadow-lg">
                <div className="text-purple-100 text-xs font-medium mb-1">THIS YEAR</div>
                <div className="text-3xl font-bold text-white mb-1">{calculateStats().thisYear}</div>
                <div className="text-purple-200 text-xs">Workouts</div>
              </div>
            </div>
            ...
            {todayWOD ? (
              <div className="bg-slate-800 rounded-xl p-5 mb-6 border border-slate-700 shadow-lg">
                ...
                <button
                  onClick={() => setCurrentView('workout')}
                  className="w-full bg-red-600 hover:bg-red-700 text-white px-5 py-3 rounded-lg font-semibold transition-colors flex items-center justify-center gap-2"
                >
                  {myResult.existingResultId ? (
                    <>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="bg-slate-800 rounded-xl p-6 text-center border border-slate-700 mb-6">
                ...
                <div className="bg-slate-700 rounded-lg p-4 text-left">
                  <p className="text-slate-300 text-xs mb-2">While you wait, you can:</p>
                  <ul className="space-y-1 text-slate-400 text-xs">
                    <li>• Review your recent workouts below</li>
                    <li>• Check back later today</li>
                    <li>• Contact your coach if it's getting late</li>
                  </ul>
                </div>
              </div>
            )}
            ...
            {calculateStats().topMovements.length > 0 && (
              <div className="bg-slate-800 rounded-lg p-6 mb-6 border border-slate-700">
                <h3 className="text-xl font-bold text-white mb-4">Most Common Movements</h3>
                <div className="space-y-3">
            ...
            <div>
              ...
              {workoutResults.length === 0 ? (
                <div className="bg-slate-800 rounded-lg p-8 text-center border border-slate-700">
                  <Dumbbell className="w-16 h-16 text-slate-600 mx-auto mb-3" />
                  <p className="text-slate-400 mb-4">No workouts logged yet. Time to crush it!</p>
                  {todayWOD && (
                    <button
                      onClick={() => setCurrentView('workout')}
        ...
        {(todayWOD || editingWorkout) ? (
          ...
          <div className="bg-slate-800 rounded-lg p-8 text-center border border-slate-700 mb-6">
            <Calendar className="w-16 h-16 text-slate-600 mx-auto mb-3" />
